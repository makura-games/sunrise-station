...
public sealed partial class PopupSystem : SharedPopupSystem
{
        ...
        EntityUid? origin = null)
    {
        ...
        // Sunrise added start - уведомляем систему иконок указания о цели popup.
        if (origin.HasValue)
    ...
    private void OnPopupEntityEvent(PopupEntityEvent ev)
    {
        ...
        if (TryComp(entity, out TransformComponent? transform))
            PopupInternal(ev.Message, ev.Type, transform.Coordinates, entity, false, GetEntity(ev.Origin)); // Sunrise-Edit
    }
