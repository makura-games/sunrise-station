    ...
    public async Task TestAllGamemodesCanStart(string presetId)
    {
        ...
        await Server.WaitPost(() =>
        {
            foreach (var ruleId in preset.Rules)
            {
                ...
                if (rule.TryGetComponent<GameRuleComponent>(out var gameRule, SEntMan.ComponentFactory))
        ...
        await Server.WaitPost(() =>
        {
            ...
            foreach (var (antag, amount) in rules)
            {
                for (var count = 0; count < amount; count++)
                {
                    SAssertAntagInitialized(antag, players[playerIndex++]);
                }
            }
        });
        ...
        // Sunrise-Edit - тестовая станция форка не гарантирует ровно один Central Command.
