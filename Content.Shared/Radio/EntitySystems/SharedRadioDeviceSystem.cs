...
using Robust.Shared.Player; // Sunrise-Edit - проверка источника речи для интеркома
using Robust.Shared.Network; // Sunrise-Edit - проверка применяется только на сервере
...
public abstract partial class SharedRadioDeviceSystem : EntitySystem
{
    ...
    [Dependency] private INetManager _net = default!; // Sunrise-Edit - серверная фильтрация интеркомов
    ...
    {
        // Sunrise added start - интеркомы не ретранслируют речь неигровых источников
        if (_net.IsServer && HasComp<IntercomComponent>(ent) && !HasComp<ActorComponent>(args.Source))
        {
            args.Cancel();
            return;
        }
