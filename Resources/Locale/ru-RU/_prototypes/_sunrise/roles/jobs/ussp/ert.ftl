ent-RandomHumanoidSpawnerERTLeaderAmber = ОБР лидер
    .suffix = Роль ОБР, Амбер
    .desc = { "" }
ent-RandomHumanoidSpawnerERTLeaderAmberLock = лидер обр
    .suffix = Роль ОБР, Янтарный, Замок
    .desc = { "" }
ent-RandomHumanoidSpawnerERTLeaderRed = ОБР лидер
    .suffix = Роль ОБР, Красный
    .desc = { ent-RandomHumanoidSpawnerERTLeaderAmber.desc }
ent-RandomHumanoidSpawnerERTLeaderRedLock = { ent-RandomHumanoidSpawnerERTLeaderAmber }
    .suffix = Роль ОБР, Красный, Замок
    .desc = { ent-RandomHumanoidSpawnerERTLeaderRed.desc }
ent-RandomHumanoidSpawnerERTLeaderGamma = { ent-RandomHumanoidSpawnerERTLeaderRed }
    .suffix = Роль ОБР, Гамма
    .desc = { ent-RandomHumanoidSpawnerERTLeaderRed.desc }
ent-RandomHumanoidSpawnerERTLeaderGammaLock = { ent-RandomHumanoidSpawnerERTLeaderRed }
    .suffix = Роль ОБР, Гамма, Замок
    .desc = { ent-RandomHumanoidSpawnerERTLeaderRed.desc }
ent-RandomHumanoidSpawnerERTSecurityAmber = ОБР офицер безопасности
    .suffix = Роль ОБР, Амбер
    .desc = { ent-RandomHumanoidSpawnerERTLeaderAmber.desc }
ent-RandomHumanoidSpawnerERTSecurityAmberLock = { ent-RandomHumanoidSpawnerERTSecurityAmber }
    .suffix = Роль ОБР, Янтарный, Замок
    .desc = { ent-RandomHumanoidSpawnerERTLeaderRed.desc }
ent-RandomHumanoidSpawnerERTSecurityRed = ОБР офицер безопасности
    .suffix = Роль ОБР, Красный
    .desc = { ent-RandomHumanoidSpawnerERTLeaderAmber.desc }
ent-RandomHumanoidSpawnerERTSecurityGamma = { ent-RandomHumanoidSpawnerERTSecurityAmber }
    .suffix = Роль ОБР, Гамма
    .desc = { ent-RandomHumanoidSpawnerERTSecurityAmber.desc }
ent-RandomHumanoidSpawnerERTSecurityGammaLock = { ent-RandomHumanoidSpawnerERTSecurityAmber }
    .suffix = Роль ОБР, Гамма, Замок
    .desc = { ent-RandomHumanoidSpawnerERTLeaderRed.desc }
ent-RandomHumanoidSpawnerERTChaplainAmber = ОБР священник
    .suffix = Роль ОБР, Амбер
    .desc = { ent-RandomHumanoidSpawnerERTLeaderAmber.desc }
ent-RandomHumanoidSpawnerERTChaplainGamma = ОБР священник
    .suffix = Роль ОБР, Гамма
    .desc = { ent-RandomHumanoidSpawnerERTChaplainAmber.desc }
ent-RandomHumanoidSpawnerERTJanitorAmber = ОБР уборщик
    .suffix = Роль ОБР, Амбер
    .desc = { ent-RandomHumanoidSpawnerERTLeaderAmber.desc }
ent-RandomHumanoidSpawnerERTJanitorGamma = ОБР уборщик
    .suffix = Роль ОБР, Красный
    .desc = { ent-RandomHumanoidSpawnerERTJanitorAmber.desc }
ent-RandomHumanoidSpawnerERTEngineerAmber = ОБР инженер
    .suffix = Роль ОБР, Амбер
    .desc = { ent-RandomHumanoidSpawnerERTLeaderAmber.desc }
ent-RandomHumanoidSpawnerERTEngineerAmberLock = { ent-RandomHumanoidSpawnerERTEngineerAmber }
    .suffix = Роль ОБР, Гамма, Замок
    .desc = { ent-RandomHumanoidSpawnerERTLeaderRed.desc }
ent-RandomHumanoidSpawnerERTEngineerGamma = ОБР инженер
    .suffix = Роль ОБР, Красный
    .desc = { ent-RandomHumanoidSpawnerERTEngineerAmber.desc }
ent-RandomHumanoidSpawnerERTEngineerGammaLock = { ent-RandomHumanoidSpawnerERTEngineerGamma }
    .suffix = Роль ОБР, Гамма, Замок
    .desc = { ent-RandomHumanoidSpawnerERTLeaderRed.desc }
ent-RandomHumanoidSpawnerERTMedicalAmber = ОБР медик
    .suffix = Роль ОБР, Амбер
    .desc = { ent-RandomHumanoidSpawnerERTLeaderAmber.desc }
ent-RandomHumanoidSpawnerERTMedicalAmberLock = { ent-RandomHumanoidSpawnerERTMedicalAmber }
    .suffix = Роль ОБР, Гамма, Замок
    .desc = { ent-RandomHumanoidSpawnerERTLeaderRed.desc }
ent-RandomHumanoidSpawnerERTMedicalGamma = ОБР медик
    .suffix = Роль ОБР, Красный
    .desc = { ent-RandomHumanoidSpawnerERTMedicalAmber.desc }
ent-RandomHumanoidSpawnerERTMedicalGammaLock = { ent-RandomHumanoidSpawnerERTMedicalGamma }
    .suffix = Роль ОБР, Гамма, Замок
    .desc = { ent-RandomHumanoidSpawnerERTLeaderRed.desc }
job-name-cburn-eva = Карантинный Офицер Центком (EVA)
job-description-cburn-eva = Отправляйтесь на станцию и устраните любые биологические угрозы. Теперь и в космосе!
job-name-cburn-leader = Лидер карантинной группы Центком
job-description-cburn-leader = Возглавьте свою команду карантинных офицеров для устранения любых биологических угроз.
job-name-ussp-crew = Экипаж СССП
job-name-ussp-officer = Офицер СССП
job-name-ussp-officer-alt = Офицер СССП (альт)
job-name-ussp-soldier = Солдат СССП
job-name-ussp-captain = Капитан СССП
ent-RandomHumanoidSpawnerERTEngineerAmberEVA = ОБР инженер
    .suffix = Роль ОБР, Амбер, EVA
    .desc = { ent-RandomHumanoidSpawnerERTEngineerAmber.desc }
