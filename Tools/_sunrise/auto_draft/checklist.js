...
function buildChecklist({ owner, repo, number, feedback, readiness, manualDraft, manualOverride }) {
  ...
  const lines = [marker, '### Готовим изменения к ревью', '',
    ...
    ...(readiness.checkItems?.length ? ['', '<details>', '<summary>Показать обязательные проверки</summary>', '',
      ...readiness.checkItems.map(item => checkbox(item.done, plain(item.name))), '', '</details>'] : []),
